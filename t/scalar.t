...
ok(QBit::Validator->new(data => [], template => {},)->has_errors, 'Default __ELEM_TYPE__ == SCALAR');
...
ok(QBit::Validator->new(data => 1234567, template => {len_max => 5},)->has_errors, 'Check len_max for scalar (error)');
...
$error = FALSE;
try {
    QBit::Validator->new(data => 'qbit', template => {__ELEM_CHECK__ => undef},);
}
catch {
    $error = TRUE;
};
...
is(QBit::Validator->new(data => 5, template => {in => 7, max => 2, __ELEM_MSG__ => 'my error msg'},)->get_error(),
...
ok(!QBit::Validator->new(data => 5, template => {POSITIVE_NUMBER},)->has_errors, 'Check "POSITIVE_NUMBER" (no error)');

ok(QBit::Validator->new(data => -5, template => {POSITIVE_NUMBER},)->has_errors, 'Check "POSITIVE_NUMBER" (error)');

is(
    QBit::Validator->new(data => -5, template => {POSITIVE_NUMBER, __ELEM_MSG__ => gettext('my error msg')},)
      ->get_error,
    gettext('my error msg'),
    'Check "POSITIVE_NUMBER" and use my error msg'
  );
